...
mapboxgl.accessToken =
  "pk.eyJ1IjoiZGFsbWF0YSIsImEiOiJjbWNpM2VkdDUxMjFzMnlwdGg5eGh4N2xoIn0.DUiFxWzL75SniPLlWoilRg";
...
const ReservarModalForm = ({
  ...
  const handleConfirmar = async () => {
    ...
    try {
      ...
      console.error(
        "Error en el proceso de reserva y obtención de factura:",
        error
      );
    ...
    <>
      <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          ...
          <Box display="grid" gridTemplateColumns="1fr 1fr" gap={2} my={1}>
            ...
            <TextField
              label="Cédula"
              defaultValue={usuario?.idUsuario}
              slotProps={{ input: { readOnly: true } }}
            />
            <TextField
              label="Correo"
          ...
          <Box display="grid" gridTemplateColumns="1fr 1fr" gap={2} my={1}>
            ...
            <TextField
              ...
              defaultValue={vehicle?.tipoVehiculo}
              slotProps={{ input: { readOnly: true } }}
            />
            <TextField
              label="Año"
              defaultValue={vehicle?.anio}
              slotProps={{ input: { readOnly: true } }}
            />
            <TextField
              label="Precio por día"
              defaultValue={`$${precioPorDia}`}
              slotProps={{ input: { readOnly: true } }}
            />
          </Box>
          ...
          <Box mt={4}>
            <Typography mb={1}>
              Selecciona el lugar de entrega y recogida:
            </Typography>
            <div
              ref={mapContainerRef}
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body2">
                Acepto los{" "}
                <Link
                  href="https://vbtvwszlngotrdoqcanb.supabase.co/storage/v1/object/public/documentos/negocio/terminos_condiciones_onboard.pdf"
                  target="_blank"
                  rel="noopener"
                >
                  términos y condiciones
                </Link>{" "}
                y me comprometo a responder por cualquier daño, infracción o uso
                indebido del vehículo reservado durante el tiempo de uso,
                aceptando total responsabilidad legal y económica derivada del
                servicio.
              </Typography>
            }
            sx={{ mt: 2 }}
      ...
      >
        <Alert
          ...
          sx={{ width: "100%" }}
        >
          ¡Reserva exitosa! Puedes revisar el estado de tu solicitud en tu
          perfil.
        </Alert>
      </Snackbar>
    </>
