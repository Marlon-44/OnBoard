...
export async function getAllReservas() {
  try {
    const response = await fetch(`${API_ROOT}/reservas`);
    if (!response.ok) {
...
export async function getReservasByUser(idUsuario) {
  try {
    const response = await fetch(`${API_ROOT}/reservas/reservas/${idUsuario}`);
    if (!response.ok) {
...
export const crearReserva = async (reservaData) => {
  try {
    const response = await axios.post(`${API_ROOT}/reservas`, reservaData);
...
export const confirmarDisponibilidad = async (idVehiculo) => {
  ...
  try {
    const response = await axios.get(`${API_ROOT}/reservas/fechas-reservadas`, {
      params: { idVehiculo },
    });
...
export const getReservasByPropietario = async (idPropietario) => {
  try {
    const response = await axios.get(
      `${API_ROOT}/reservas/propietario/${idPropietario}`
    );
    return response.data;
  } catch (error) {
    console.error(
      `Error al obtener las reservas del propietario ${idPropietario}:`,
      error
    );
