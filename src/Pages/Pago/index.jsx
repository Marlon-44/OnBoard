...
const Pago = () => {
  ...
  useEffect(() => {
    const fetchFactura = async () => {
      try {
        const res = await fetch(`${API_ROOT}/facturas/${idFactura}`);
        ...
        .Buttons({
          createOrder: async () => {
            const res = await fetch(
              `${API_ROOT}/pagos/crear?idFactura=${factura.idFactura}`,
              {
                method: "POST",
              }
            );
          ...
          onApprove: async (data) => {
            try {
              const res = await fetch(
                `${API_ROOT}/pagos/capturar?orderId=${data.orderID}`,
                {
                  method: "POST",
                }
              );
              ...
              if (result.status === "COMPLETED") {
                ...
                setError(
                  "Se requiere una acción adicional del usuario para completar el pago."
                );
              } else {
  ...
  return (
    <Box maxWidth="800px" mx="auto" mt={4}>
      ...
      <Paper sx={{ p: 3 }}>
        <Typography
          variant="subtitle1"
          style={{ color: "#000", fontFamily: "Manrope" }}
        >
